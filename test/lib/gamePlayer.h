...
public:
	...
	virtual int nextStep(const GameBoard &obj)=0;
private:
...
public:
	...
	virtual int nextStep(const GameBoard &obj);
};
...
public:
 ...
 //    	Bound(const A& a) : alpha(a.alpha), beta(a.beta), depth(a.depth), value(a.value) {} // user-defined copy ctor
	...
	virtual int nextStep(const GameBoard &obj);
private:
	GameBoard* getNewState(GameBoard* obj, int depth, int step);
	int unitScore(GameBoard::TYPE winner, int depth);
